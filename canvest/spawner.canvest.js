...
describe('spawner randomizeInterval tests', async () => {
    ...
    it('should return different values each call', async () => {
        let random = true;
        let interval = 0;

        for (let i = 0; i < 10; i++) {
            let hold = spawner.randomizeInterval()
            if (interval === hold) random = false;
            else interval = hold;
        }

        expect(random).to.equal(true);
    });
});
...
describe('spawner spawnIron tests', async () => {
    ...
    it('should set random y positions for single irons', async () => {
        let yVals = [];
        spawner.ironRandScale = -55.5;
        spawner.ironRandAdd = 163.2;
        for (let i = 0; i < 10; i++) {
            spawner.obstacles = [];
            spawner.spawnIron();

            if (spawner.obstacles.length === 1) {
                yVals.push(spawner.obstacles[0].y);
            }
        }

        for (let i = 0; i < yVals.length - 1; i++) {
            expect(yVals[i]).to.not.equal(yVals[i + 1]);
        }
    });
});
